 ...
 * ```
 */
export function generateUniqueDeckName(baseName: string, maxLength = 30): string {
  const now = new Date();
  const timestamp =
  ...
  if (maxBaseLength <= 0) {
    throw new Error(`maxLength (${maxLength}) must be greater than timestamp length (${timestampLength})`);
  }
 ...
 * ```
 */
export function generateUniqueDeckNameShort(baseName: string, maxLength = 30): string {
  ...
  if (maxBaseLength <= 0) {
    throw new Error(`maxLength (${maxLength}) must be greater than timestamp length (${timestamp.length})`);
  }
