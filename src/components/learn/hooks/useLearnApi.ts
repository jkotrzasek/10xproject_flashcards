...
export function useLearnApi() {
   ...
   */
  const fetchLearnFlashcards = useCallback(async (
    deckId: number,
    limit: number = 50
  ): Promise<{ data: LearnResponseDto | null; error: string | null }> => {
    setIsLoading(true);
    setError(null);
   ...
   */
  const submitReviews = useCallback(async (
